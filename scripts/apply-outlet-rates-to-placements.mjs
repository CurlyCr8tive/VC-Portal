...
const placements = await rest("placements?select=id,client_id,publication,ave_value,ave_auto_calculated");
const targets = [];
const noRate = [];
for (const p of placements) {
  ...
  targets.push({ id: p.id, client: clientName[p.client_id], publication: p.publication, value: rate });
}
...
for (const t of targets) console.log(`   + $${t.value.toLocaleString(undefined, { maximumFractionDigits: 0 }).padStart(9)}  ${t.client} — ${t.publication}`);
if (noRate.length) {
...
for (const t of targets) {
  await rest(`placements?id=eq.${t.id}`, {
    method: "PATCH",
    body: { ave_value: Number(t.value.toFixed(2)), ave_auto_calculated: true },
